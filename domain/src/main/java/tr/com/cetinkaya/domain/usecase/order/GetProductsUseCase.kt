...
) : UseCase<GetProductsUseCase.Request, GetProductsUseCase.Response>(configuration) {

    override fun process(request: Request): Flow<Response> =
        orderRepository.getPlannedGoodsAcceptanceProducts(request.documents, request.warehouseNumber).map { Response(it) }
